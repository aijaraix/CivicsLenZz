...
# Fix the MapFitter so it only zooms in if the user explicitly searched an address.
# If they are just filtering by "Local" nationwide, keep the national view.
...
function MapFitter({ officials, userLocation }: { officials: TrackedOfficial[], userLocation: [number, number] | null }) {
    ...
    useEffect(() => {
        if (officials.length === 0 && !userLocation) return;
